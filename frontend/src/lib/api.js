...
export const updateCard = (cardId, data) => api.put(`/api/cards/${cardId}`, data).then(res => res.data);
export const deleteCard = (cardId) => api.delete(`/api/cards/${cardId}`).then(res => res.data);

export const getTodayCards = () => api.get('/api/today').then(res => res.data);
export const submitReview = (cardId, rating, durationSeconds = 0) => api.post('/api/review', { card_id: cardId, rating, duration_seconds: durationSeconds }).then(res => res.data);
